 ...
 * 目标：
 * - 显示倍率、家宽/商宽、IPLC/IEPL/BGP 等多标签
 * - 多标签同时显示
 * - 输出格式：地区 + 序号 + 标签 + 倍率（用 fgf 分隔，序号与地区之间用 sn 分隔）
 * - 上标倍率 ˣ²/ˣ¹⁰/ˣ⁵⁰ 自动转为 2倍率/10倍率/50倍率
 * - bl 开启时：未写倍率默认补 1.0倍率；5.00 -> 5.0；4.25 -> 4.25
 * - 现在用的参数（推荐）#blgd&bl&fgf=-&sn=
 * fgf=-：字段分隔用 -
...
// sn 参数：sn=（空）时 Sub-Store 可能给 true，这里强制空串
const XHFGF =
  inArg.sn === undefined ? " " :
...
// 标签识别（可多标签）
const regexArray = [
  /IPLC/i, /IEPL/i, /\bBGP\b/i,
  /核心/, /边缘/, /高级/, /标准/, /实验/,
  /(商宽|biz)/i,
  /(家宽|home|res|fam|🏠)/i,
  /游戏|game/i, /购物/, /专线/, /LB/, /cloudflare/i, /\budp\b/i, /\bgpt\b/i, /\budpn\b/i
];
...
const valueArray = [
  ...
  "商宽",
  "家宽",
  "游戏","购物","专线","LB","CF","UDP","GPT","UDPN"
];

const nameblnx = /(高倍|(?!1)2+(x|倍|倍率)|ˣ[⁰¹²³⁴⁵⁶⁷⁸⁹]+)/i;
const namenx  = /(高倍|(?!1)(0\.|\d)+(x|倍|倍率)|ˣ[⁰¹²³⁴⁵⁶⁷⁸⁹]+)/i;
...
function parseSuperXRate(text) {
  const m = String(text).match(/ˣ([⁰¹²³⁴⁵⁶⁷⁸⁹]+)/);
  if (!m) return null;
  const digits = m[1].split("").map((c) => SUPER_NUM_MAP[c] ?? "").join("");
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 1) 标签（blgd）
    let rateFromBlgd = ""; // 由 ˣ上标 得到的倍率，例如 "50倍率"
    const tags = [];

    if (blgd) {
      const xRate = parseSuperXRate(e.name);
      if (xRate !== null) {
        // 上标倍率输出不带小数：50倍率
        rateFromBlgd = String(xRate) + "倍率";
      }
      ...
      if (uniqTags.length) {
        // 标签之间用 FGF 分隔（你要的效果：IPLC-家宽）
        ikeys = uniqTags.join(FGF);
      }
    }
    ...
    if (bl) {
      ...
      if (match) {
        ...
        ikey = formatRate(rev) + "倍率";
      } else if (rateFromBlgd) {
        ikey = rateFromBlgd; // ˣ⁵⁰ -> 50倍率
      } else {
        ikey = "1.0倍率";
      }
    }
    ...
    if (findKey?.[1]) {
      ...
      // 尾巴：保留关键字 + 标签 + 倍率（按你要求的顺序）
      const tailParts = uniq([retainKey, ikeys, ikey].filter((k) => k !== ""));

      e.__baseName = baseParts.join(FGF);
      e.__tailName = tailParts.join(FGF);

      // 临时名（jxh 会重排成：基名 + 序号 + 尾巴）
...
// 编号：按 __baseName 分组计数，输出：base + sn + 01 + fgf + tail
function jxh(pro) {
